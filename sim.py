...
class Simulation:
    ...
    def __init__(self):
        os.add_dll_directory("C:\\Program Files\\Lumerical\\v202\\api\\python") # Para evitar um erro ao chamar a API (não sei se precisa mesmo)
        self.lumapi = importlib.util.spec_from_file_location("lumapi", "C:\\Program Files\\Lumerical\\v202\\api\\python\\lumapi.py").loader.load_module() # Carrega a API, localizada nesse path ai (pode ser diferente para cada computador)
        self.objects = []
        self.p= 0.9e-6
    
    def create(self, save_path, n_freq=300,resolution=8,wl_start=10.0e-6,wl_stop=30.0e-6): # Função que cria a simulação e a estrutura inicial
        ...
        self.sim.set("mesh refinement", "conformal variant 1")  # Define esse paremetro para conformal variant 1 (aparentemente isso é bom pra metal)
        ...
        self.sim.setglobalmonitor("frequency points", n_freq) # Faz parte da resolução da simulação
    ...
    def run(self,data_name):            # Roda a simulação
        ...
        reflected = self.sim.getresult("reflected","T")     # Resultado de quanto foi refletido
        
        r_lambda = reflected['lambda']      # Lista de comprimentos de onda da luz
        r_lambda = r_lambda[:,0]
        r_T = reflected['T']                # Lista com a refletancia em cada ponto
        
        transmited = self.sim.getresult("transmited","T")    # Resultado de quanto foi transmitido
        ...
        data = {                                # Dicionario com os dados para salvar em um dataset.json
            ...
            "Y": np.ndarray.tolist(absorved)    # Pontos de absorção (porém o lumerical devolve os dados em array do numpy ent tem que transformar em lista)
        }
        x = r_lambda
        sum_90 = [0]
        sum_98 = [0]
        ...
        for i in range(len(data["Y"])):
            item = data["Y"][i]
            if i != 0:
                integral += ((item + data["Y"][i - 1]) * (x[i - 1] - x[i])) * 0.5 * 1.0e6
            if item >= 0.9:
                if i != 0:
                    if data['Y'][i-1] > 0.9:
                        sum_90[k1] += 1
                        ...
                        k1+=1
                        sum_90[k1] += 1
                    int_90 = int_90 + ((item + data["Y"][i - 1]) * (x[i - 1] - x[i])) * 0.5 * 1.0e6
                if item >= 0.98:
                    if i != 0:
                        if data['Y'][i - 1] > 0.9:
                            sum_98[k2] += 1
                            ...
                            k2 += 1
                            sum_98[k2] += 1
                        int_98 = int_98 + ((item + data["Y"][i - 1]) * (x[i - 1] - x[i])) * 0.5 * 1.0e6

        ans = {"sum_90": sum_90, "sum_98": sum_98, "int_90": int_90, "int_98": int_98, "integral": integral}

        with open(data_name, "a") as f:                         # Abre o arquivo de dados
            if os.stat(data_name).st_size == 0:                 # Testa se o arquivo do dataset esta vazio
                json.dump(np.ndarray.tolist(r_lambda), f)       # E então cria o vetor de comprimentos de onda (eixo x)
                f.write("\n")                                   # Pula para proxima linha
            f.close()                                           # Fecha o arquivo

        with open(data_name, "a") as f:                         # Abre de novo (Agora certamente o arquivo existe e tem o eixo x na 1° linha)
